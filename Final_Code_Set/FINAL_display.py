...
def snake(user, score) :
    now = datetime.now()
    link = os.path.join(os.path.dirname(os.path.abspath(__file__)), ('userdata/'+user+'.dat'))
...
def end(__name, __user, __score) :
    try :
        os.makedirs(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'userdata'), exist_ok=True)
